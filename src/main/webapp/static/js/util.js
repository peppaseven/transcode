...
$(function() {
	...
	function g() {
		...
		if (j.length == 0) {
			j = $(
					"<div id='userQuickInfo' class='shadow_1 radius3' style='display:none;'></div>")
					.appendTo("body")
		}
		return j
	}
...
Util.loading = function(b) {
	...
	var a = $(
			"<div id='top_loading_tip' class='loadingTop'><p><b>" + b.content
					+ "</b></p></div>").appendTo("body");
	if (b.model) {
	...
	if (typeof (b.show) == "number") {
		$("#top_loading_tip").delay(b.delay).show(0, function() {
			if (b.model) {
				$("#dialog_model").show()
			}
		}).delay(b.show).fadeOut(500, function() {
			if (b.model) {
				$("#dialog_model").hide()
			}
		})
...
$.fn.submitForm = function(opt) {
	...
	form
			.append("<input type='hidden' name='submitFormByHiddenFrame' id='submitFormByHiddenFrameParam' value='hiddenFrame'/>");
	form[0].submit();
...
Util.loginWindow = function(c, b) {
	...
	if (c == "open") {
		...
		var a = $("<div id='loginWindow' class='loginWindow'></div>").appendTo(
				"body");
		a
				.append("<div id='loginWindow-content' class='loginWindow-content'><img src='/images/ajaxload.gif' style='margin:80px 0px 0px 45%'/></div>");
...
Util.globalTopTip = function(d, c, b) {
	...
	e = $(
			'<div id="global_top_dialog" class="global_top_dialog"><div class="left_arrow"></div>'
					+ d + '<div class="right_arrow"></div></div>').appendTo(
			"body");
	e.addClass(c);
	var a = e.outerWidth();
	if ($("#header").length == 0) {
...
(function(c) {
			...
			c(window).bind(
					...
					function() {
						...
						if (g.fixed) {
							...
							m = (c(window).height() - e) / 2
									+ c(document).scrollTop() + "px"
						}
	...
	c.confirm = function(e) {
		...
		if (!f.length) {
			f = c(
					"<div id='global_confirm_window' title='请确认'><div class='msg'></div><div class='buttons'><span class='button default okbtn'>确定</span>&nbsp;&nbsp;<span class='button cancelbtn'>取消</span></div></div>")
					.appendTo("body")
		}
		...
		f.dialog();
		f.find(".okbtn").unbind().bind("click", function() {
			f.dialog("close");
			if (e.onConfirm) {
				e.onConfirm()
			}
		});
		f.find(".cancelbtn").unbind().bind("click", function() {
			f.dialog("close");
			if (e.onCancel) {
				e.onCancel()
			}
	...
	c.fn.popMenu = function(e) {
		...
		if (f.autoClose) {
			...
			c(document).bind("mouseup.popmenu", function() {
				k.popMenu("close");
		...
		c(window).bind("resize.popmenu", function() {
			k.popMenu(e)
		});
		k.show();
		var h = 0;
		...
		if (f.autoPosition
				&& g + f.offsetY + k.outerHeight() > c(window).height()
						+ c(document).scrollTop()) {
			k.css({
				top : c(window).height() - k.outerHeight()
						+ c(document).scrollTop(),
				left : h + f.offsetX
			})
		} else {
	...
	c.fn.spread = function(j) {
		...
		var e = c(q[0]), C = e.parent();
		var A = parseInt(e.css("padding-left").replace("px", "")), v = parseInt(e
				.css("padding-right").replace("px", "")), t = parseInt(e.css(
				"padding-top").replace("px", "")), f = parseInt(e.css(
				"padding-bottom").replace("px", "")), u = parseInt(C.css(
				"padding-left").replace("px", "")), o = parseInt(C.css(
				"padding-right").replace("px", "")), n = parseInt(C.css(
				"padding-top").replace("px", "")), B = parseInt(C.css(
		...
		var g = c.extend(l, j);
		var m = g.w, x = g.h, y = g.ml, r = g.mt, z = g.maxWidth, p = g.s;
		c.each(q, function(s, w) {
			var h = c(w);
			h.css({
				top : "-" + x + "px",
				left : 0
	...
	c.simpleAlert = function(j, h, f) {
		...
		var e = c("<div id='simplealert' class='simplealert'></div>").appendTo(
				"body");
		var g = "<div class='" + i + "'>";
		if (h == "loading") {
			g += "<img src='/images/default/designer/loading.gif' style='margin:10px 0px 0px 12px'/>"
		}
		g += "</div><div class='simplealert-msg'>" + j
				+ "</div><div class='simplealert-right'></div>";
		e.html(g);
		e.css("top", (c(window).height() - e.height()) / 2
				+ c(window).scrollTop() + "px");
		e.css("left", (c(window).width() - e.width()) / 2
				+ c(window).scrollLeft() + "px");
		e.show();
		if (f != "no") {
			setTimeout(function() {
				e.fadeOut(200)
	...
	c.fn.tooltip = function(g, f, k, l, e) {
		...
		f = f ? f : "warning";
		if (f != "none") {
			g = "<img src='/images/icon/ico-"
					+ f
					+ ".png' style='vertical-align:middle;margin-right:5px;'/><span>"
					+ g + "</span>"
		}
		...
		c("body").append(
				'<p id="p_toolTip" class="tool-tip radius3">' + g + "</p>");
		h = c("#p_toolTip");
		if (l == "top") {
			i = c(this).offset().top - h.height() - 20 + "px";
			j = c(this).offset().left + c(this).width() / 2 - h.width() / 2
					+ "px"
		} else {
			if (l == "bottom") {
				...
				if (l == "left") {
					i = c(this).offset().top + c(this).height() / 2
							- h.height() / 2 + "px";
					j = c(this).offset().left - h.width() - 20 + "px"
				} else {
					if (l == "right") {
						i = c(this).offset().top + h.height() / 2 - h.height()
								/ 2 + "px";
	...
	c.fn.draggable = function(e) {
		...
		var f = c.extend(g, e);
		c(this).unbind("dragstart").bind("dragstart", function() {
			return false
		});
		c(this).unbind("mousedown.drag").bind(
				"mousedown.drag",
				function(i) {
					c(document).bind("selectstart", function() {
						return false
					});
					var l = i.pageX;
					var j = i.pageY;
					...
					c(document).bind(
							...
							function(p) {
								...
								if (f.bounding) {
									...
									if (o > m
											&& n > q
											&& o < m + f.bounding.outerWidth()
													- f.target.outerWidth()
											&& n < q + f.bounding.outerHeight()
													- f.target.outerHeight()) {
										f.target.offset({
											left : o,
											top : n
										})
					...
					c(document).bind("mouseup.drag", function(m) {
	...
	c.fn.switchbutton = function(f) {
		...
		var g = c.extend(l, f);
		var k = c(this), e = k.find(".switch"), i = k.find(".switch-left"), h = k
				.find(".switch-right");
		k.css({
			width : g.width,
			height : g.height,
	...
	c.fn.pagination = function(o, k, n, j) {
		...
		if (g <= k - 2) {
			f += "<a class='disabled ellipsis'>...</a><a p='" + k + "'>" + k
					+ "</a>"
		} else {
			if (g <= k - 1) {
				f += "<a p='" + k + "'>" + k + "</a>"
		...
		l.html(f);
		if (n) {
			l.find("a[p]").bind("click", function() {
				var i = c(this).attr("p");
				n(i)
			})
		}
	...
	c.fn.streamInput = function(u) {
		...
		c(f.target)
				...
				.on(
						...
						function(A) {
							...
							z.deleteContents();
							try {
								var B = x.getData("text/html")
										|| x.getData("text/plain").replace(
												/</g, "&lt;").replace(/>/g,
												"&gt;");
								B = B.replace(/<(\/)?(html|body)(\s|\S)+?>/g,
										"").replace(/(<!--.+?-->)|(\r\n)/g, "");
								var v = c("<div>" + B + "</div>");
								q(v);
								t(v);
		...
		c(f.face)
				...
				.on(
						...
						function() {
							...
							var v = a.target = c("div[stream_id='"
									+ c(this).attr("for_stream") + "']");
							v.focus();
							if (a.range) {
								var x = window.getSelection();
								x.removeAllRanges();
		...
		c("#faces_dialog ul li").off("click.stream").on(
				"click.stream",
				function(z) {
					c("#faces_dialog").popMenu("close");
					var w = c(a.target), x = c(this).index() + 1, A;
					A = "<img class='ico-face' src='/images/default/faces/" + x
							+ ".gif'>";
		...
		function q(y) {
			try {
				var v = y.html().replace(/^\s+/g, " ").replace(/(\S)\s+(\S)?/g,
						"$1 $2");
				y.html(v);
		...
		function t(w) {
			...
			v.each(function(x, y) {
				c(y).replaceWith(
						c("<img class='" + c(y).attr("class") + "' src='"
								+ c(y).attr("src") + "'>"))
			})
		}
		function g(w) {
		...
		function j(C) {
			var B = C.cloneNode(true), w = B.childNodes.length, y = 0, x = 0;
			if (w > 3 && c(B).children(".paste-cont").length > 0) {
				for (var z = 0; z < w; z++) {
					...
					if (A.nodeName == "SPAN") {
						y = w - z - 1;
						if (z < w - 1
								&& B.childNodes[z + 1].nodeName == "#text") {
		...
		function n(w, x, B, D) {
			var A = w.html(), y = document.createElement("div"), C = document
					.createDocumentFragment(), z, v;
			if (A) {
				y.innerHTML = A;
				while ((z = y.firstChild)) {
...
function initTemplateCategorySelect() {
	...
	$(".item-container").die().on("click", function() {
					...
					function(e) {
						if (a == "my_template") {
							...
							if ($(".item-container[chartId='']").length == 0) {
								$("#template-container")
										.append(
												'<div class="item-container template_selected radius3" chartId=""><div></div>空模板</div>')
							}
						}
						for (var c = 0; c < e.templates.length; c++) {
							...
							$("#template-container")
									.append(
											'<div define="'
													+ b.chartId
													+ '" class="item-container radius3"><div><img src="/chart_image/thumb/'
													+ b.thumbnail
													+ '.png"/></div>' + b.title
													+ "</div>")
						}
...
function templateSelected(g) {
	...
	if (g == "template") {
		a = "/diagraming/new?template=" + c + "&category=" + f
				+ "&istemplate=true"
	}
	if (globalNewTeamId) {
